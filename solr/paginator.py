...
#        self.hl_fields = []
#        if getattr(result, 'highlighting', False):
#            self.highlight = True
#            print "hl.fl-------> ", self.params['hl.fl']
#            for field in self.params['hl.fl'].split(','):
#                self.hl_fields.append(field)  
#        
    ...
    def _fetch_page(self, start=0):
        ...
        new_params['rows'] = self.page_size

        return self.conn.query(**new_params) 
        #return self.conn.query(highlight=self.highlight, fields=self.hl_fields, **new_params) 
    ...
    def page(self, page_num=1):
        ...
        start = (page_num - 1) * int(self.page_size)
